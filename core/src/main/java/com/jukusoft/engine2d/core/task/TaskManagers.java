...
public class TaskManagers {
    ...
    public static void register(int threadID, TaskManager taskManager) {
        ...
        if (threadID >= MAX_THREADS)
            throw new IllegalArgumentException("max allowed threadID is " + MAX_THREADS + ", current value: " + threadID);
        Objects.requireNonNull(taskManager);

        if (taskManagers[threadID] != null)
            throw new IllegalStateException("task manager with threadID " + threadID + " was already registered");
